#!/bin/bash
#SBATCH --job-name=delphi_v3_train
#SBATCH --nodes=1
#SBATCH --ntasks=1
...
mkdir -p $PIPE/logs $DELPHI/checkpoints/mimic_v3

# Symlink data dir so train.py can find data/mimic_data_v3/
if [ ! -L $DELPHI/data/mimic_data_v3 ]; then
    ln -s $PIPE/data/mimic_data_v3 $DELPHI/data/mimic_data_v3
fi

# Resolve vocab_size from preprocessed meta.pkl (raw vocab + 1 for get_batch shift)
VOCAB_SIZE=$($PYBIN -c "import pickle; m=pickle.load(open('$PIPE/data/mimic_data_v3/meta.pkl','rb')); print(m['vocab_size']+1)")
echo "=== Delphi v3 Training ==="
echo "Node: $(hostname)  GPU: $CUDA_VISIBLE_DEVICES"
echo "Vocab size: $VOCAB_SIZE"
echo ""

cd $DELPHI
$PYBIN train.py $PIPE/config/train_delphi_mimic_v3.py \
    --vocab_size=$VOCAB_SIZE \
    --out_dir=checkpoints/mimic_v3

echo ""
echo "=== Training complete. Checkpoint in $DELPHI/checkpoints/mimic_v3/ ==="
