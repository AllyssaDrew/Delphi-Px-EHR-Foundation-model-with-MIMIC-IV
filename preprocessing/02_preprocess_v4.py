"""
Phase 2 (v4) - MIMIC-IV → Delphi binary format
Changes from v3:
  2.2  Post-2015 ICD-10 filter: only use ICD codes from admissions with
       admittime >= 2015-10-01 (US ICD-10 adoption date). GEMs mapping
       is eliminated entirely — all codes are native ICD-10.

Token layout (stored space → model space after +1 shift in get_batch):
  Stored → Model
  (PAD/-1) → 0  Padding
  (0, dynamic) → 1  No event (synthetic)
  1 → 2          [NoEvent placeholder for Global Memory summary positions]
  2 → 3          Female
  3 → 4          Male
  4–6 → 5–7     BMI (low / mid / high)
  7 → 8          ICU_admission
  8 → 9          ED_admission
  9..D-1 → 10+  ICD-10 3-char codes  (sorted, ≥MIN_CODE_COUNT; first-occurrence only)
  D → D+1        Death  (DEATH_TOKEN)
...
OUT_DIR = os.path.join(BASE, 'data/mimic_data_v4')
...
# v4: only use ICD codes from admissions on or after ICD-10 adoption date
...
# ══════════════════════════════════════════════════════════════════════════════
print("Step 1: Loading MIMIC-IV tables ...")
...
# ══════════════════════════════════════════════════════════════════════════════
# Step 2 — Patient filtering (Option B)
# ══════════════════════════════════════════════════════════════════════════════
...
# ══════════════════════════════════════════════════════════════════════════════
# Step 3 — ICD code events (post-2015, native ICD-10 only; no GEMs)
# ══════════════════════════════════════════════════════════════════════════════
print("\nStep 3: Building ICD code events (post-2015 ICD-10 only) ...")

# Admissions on or after ICD-10 adoption date
adm_post15 = adm_age[adm_age['admittime'] >= ICD10_CUTOFF][['subject_id','hadm_id','age_days']]
n_all_hadm   = adm_age['hadm_id'].nunique()
n_post15_hadm = adm_post15['hadm_id'].nunique()
print(f"  Admissions (all):     {n_all_hadm:>8,}")
print(f"  Admissions post-2015: {n_post15_hadm:>8,}  ({100*n_post15_hadm/n_all_hadm:.1f}%)")

# Pre-check: warn if many patients lose all ICD events
pats_with_any_icd = diagnoses['subject_id'].unique()
diag_post15_pats  = diagnoses[diagnoses['hadm_id'].isin(adm_post15['hadm_id'])]['subject_id'].unique()
n_lost = len(set(pats_with_any_icd) - set(diag_post15_pats))
frac_lost = n_lost / max(len(pats_with_any_icd), 1)
if frac_lost > 0.10:
    print(f"  WARNING: {n_lost:,} patients ({100*frac_lost:.1f}%) have no ICD codes "
          f"after post-2015 filter (expected if mostly pre-2015 admissions).")
else:
    print(f"  Patients with no ICD events after filter: {n_lost:,} ({100*frac_lost:.1f}%)")

# ICD-10 diagnoses from post-2015 admissions
d10 = diagnoses[
    (diagnoses['icd_version'] == 10) &
    (diagnoses['hadm_id'].isin(adm_post15['hadm_id']))
].copy()
d10['code3'] = d10['icd_code'].str[:3].str.upper()
diag10 = d10.merge(adm_post15[['subject_id','hadm_id','age_days']],
                   on=['subject_id','hadm_id'], how='inner')
diag10 = diag10[['subject_id','age_days','code3']]

# ICD-10-PCS procedures from post-2015 admissions
p10 = procedures[
    (procedures['icd_version'] == 10) &
    (procedures['hadm_id'].isin(adm_post15['hadm_id']))
].copy()
...
all_disease = pd.concat([diag10, p10], ignore_index=True)
...
# First-occurrence deduplication: per patient, keep only earliest code occurrence
...
# Vocabulary: frequency threshold 25
...
# ══════════════════════════════════════════════════════════════════════════════
# Step 4 — ICU and ED tokens from transfers
# ══════════════════════════════════════════════════════════════════════════════
...
# ══════════════════════════════════════════════════════════════════════════════
# Step 6 — Lab event tokens
# ══════════════════════════════════════════════════════════════════════════════
print("\nStep 6: Building lab event tokens ...")
...
for lab_idx, (itemids, name, (thresh_lo, thresh_hi), unit) in enumerate(LAB_ITEMS):
    ...
    def discretise(v):
        if v < thresh_lo:  return 0
        if v < thresh_hi:  return 1
        return 2
...
# Step 12 — meta.pkl
# ══════════════════════════════════════════════════════════════════════════════
# ignore_tokens (model space, after +1 shift):
#   0=PAD, 2=GM_placeholder(stored 1), 3=F(stored 2), 4=M(stored 3),
#   5-7=BMI(stored 4-6), 8=ICU(stored 7) excluded? No — keep ICU in loss.
# Matching v3 convention: [0, 2, 3, 4, 5, 6, 7]
# In v4 model space (RESERVED=9, stored 9 → model 10):
#   stored 6 (ICU token_id=7) → model 8 — NOT in ignore list (clinical event)
#   stored 7 (ED token_id=8) → model 9  — NOT in ignore list
# The stored token_ids written above are: sex=2/3, bmi=4/5/6, icu=7, ed=8.
# After +1 shift: sex=3/4, bmi=5/6/7, icu=8, ed=9. ICD starts at model 10.
# ignore_tokens_shifted excludes 0,2,3,4,5,6,7 (PAD, GM_placeholder, sex, BMI).
...
meta = {
    ...
    'icd10_cutoff':    str(ICD10_CUTOFF.date()),
    'split': {
    ...
    'improvements': [
        'post2015_icd10_only', 'no_gems_mapping',
        'first_occurrence_dedup', 'freq_threshold_25', 'lab_tokens',
    ],
}
...
# ══════════════════════════════════════════════════════════════════════════════
print("\n" + "="*62)
print("Phase 2 v4 COMPLETE — Summary")
print("="*62)
print(f"  Vocab size:           {vocab_size}")
print(f"    ICD codes:          {len(codes_kept)}"
      f"  (post-2015 ICD-10 only, freq≥{MIN_CODE_COUNT})")
...
print()
print(f"  Next → config/train_delphi_mimic_v4.py")
print(f"    vocab_size   = {vocab_size}   # raw; model needs +1 = {vocab_size+1}")
print(f"    ignore_tokens= {ignore_tokens_shifted}")
print(f"    block_size   = 128  (n_summary=4 prepended → max_seq_len=132)")
